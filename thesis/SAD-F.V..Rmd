...
Possiamo notare che la somma degli elementi su ogni colonna si avvicina di molto a 100, non è esattamente 100 causa arrotondamento.
...
Il rapporto più alto viene registrato in ancora in Lombardia(15.08), il rapporto più basso viene registrato dalla Valle d'Aosta con un rapporto di laureati magistrali pari a 0.01.
...
La regione con il rapporto più alto è la Campania con 11.68 mentre la regione con il rapproto più basso è la Valle d'Aosta con 0.01.
...
Di seguito viene mostrato lo script fatto per mostrare lo scatterplot della retta di regressione, per una migliore visualizzazione i dati riguardanti i rapporti triennali e magistrali sono stati scalati utilizzando la funzione **scale()**.
```{r echo=TRUE}
...
* non ci sono outlier
* per i laureati triennali, magistrali e a ciclo unico la maggior parte dei valori si trova sopra la mediana
* i dati risultano avare una maggiore dispersione nella colonna dei laureati magistrali

# 6. Analisi dei cluster
L'analisi dei cluster è una metodologia che permette di raggruppare un campione ampio in vari sottoinsiemi più piccoli detti cluster. L'obiettivo di questa analisi è quello di **raggruppare** fra loro elementi simili.

## 6.1 Costruizione 
Abbiamo detto che raggruppiamo in base alla **somiglianza** degli elementi, per calcolare questa somiglianza applichiamo i metodi delle distanze. Le distanze tra tutte le possibili coppie di unità vengono inserite in una matrice simmetrica _D_ di cardinalità _n x n_. Il metodo che che utilizzerò per calcolare le distanze è quello della **metrica Euclidea**.  
...
L'**enumerazione completa** è molto onerosa dal punto di vista computazionale, in quanto questo metodo cerca di ottimizzare(minimizzare) una fissata funzione obiettivo per ogni possibile partizione degli individui all'interno di un cluster.  
...
la densità di __popolazioneAtleti__ viene calcolata dal seguente script R
...
Possiamo stimare i parametri non noti con due metodi, **metodo dei momenti** e **metodo della massima verosomiglianza**. Dato che vogliamo stimare i paramentri non noti di una distribuzione normale la scelta
...
```{r echo=FALSE}
print(paste("stimatore valore medio mu =", round(mu, digits = 2), ", stimatore varianza sigma^2 =", round(sigmaQ, digits = 2)))
```
...
```{r}
alpha <- 1 - 0.95 #grado di confidenza
...
Il grado di fiducia assegnato in questo caso è 0.95, in seguito è stata calcolata la densità con il metodo del **$\chi^2$ con n gradi di libertà** dove n è pari alla grandezza del campione(__size__). La densità è stata calcolata sia per il limite superiore che per quello inferiore, i risultati sono riportati nello script soprastante. Infine è stata calcolata la stima dell'intervallo di confidenza di grado 0.95, l'intervallo va da 7.21 a 12.26 e la varianza(9.25) è compresa nell'intervallo. Di seguito viene riportato il grafico
